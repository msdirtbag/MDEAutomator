...
MDEAutomator is designed for flexible integration with your security and automation workflows. While there is no official frontend GUI, you can interact with its Azure Function Apps using HTTPS from any tool or platform that supports REST API calls, such as PowerShell, Logic Apps, Power Apps, custom web apps, or even direct API clients like Postman.

Example request payloads are available in the [`tests`](https://github.com/msdirtbag/MDEAutomator/tree/main/tests) folder.

![Portal](./media/portal.png)

If you are new to REST APIs, see: [REST Fundamentals](https://www.geeksforgeeks.org/rest-api-introduction/#)
...
Each Azure Function App exposes one or more HTTP endpoints. The general format is:

```
https://<APP_NAME>.azurewebsites.net/api/<FUNCTION_NAME>
```

For example, to call the MDEDispatcher function:
```
https://my-mdeautomator-app.azurewebsites.net/api/MDEDispatcher
```

#### 2. Authentication

Most functions require an access key unless set to "anonymous" authorization. You can provide the key in two ways:

- **Query String:**  
  Append `?code=<FUNCTION_KEY>` to the URL.
  ```
  https://<APP_NAME>.azurewebsites.net/api/<FUNCTION_NAME>?code=<FUNCTION_KEY>
  ```
...
Below are sample JSON payloads for common function calls. All requests should use the `POST` method and `Content-Type: application/json`.
...
```json
{
  "TenantId": "<your-tenant-guid>",
  "Queries": [
    "DeviceProcessEvents | where FileName == 'mimikatz.exe' | limit 10",
...
```powershell
$functionUrl = "https://<APP_NAME>.azurewebsites.net/api/MDEDispatcher?code=<FUNCTION_KEY>"
$body = @{
    TenantId = "<your-tenant-guid>"
    Function = "InvokeMachineIsolation"
...
```bash
curl -X POST "https://<APP_NAME>.azurewebsites.net/api/MDEDispatcher?code=<FUNCTION_KEY>" ^
  -H "Content-Type: application/json" ^
  -d "{\"TenantId\":\"<your-tenant-guid>\",\"Function\":\"InvokeMachineIsolation\",\"DeviceIds\":[\"<device-id-1>\"]}"
```

#### Logic Apps / PowerApps

- Use the HTTP action.
- Set method to POST.
- Set URI to your function endpoint with the function key.
- Set headers: `Content-Type: application/json`.
- Paste your JSON payload in the body.

---

### Troubleshooting

- **401 Unauthorized:** Check your function key and ensure it's included in the request.
- **404 Not Found:** Verify the function name and app URL.
- **400 Bad Request:** Validate your JSON payload and required fields.
- **500 Internal Server Error:** Check Azure Function logs for more details.
